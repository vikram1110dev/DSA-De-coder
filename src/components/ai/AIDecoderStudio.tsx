    ...
    <div className="surface overflow-hidden">
      ...
      >
        <div className="flex items-center gap-3">
          <span className="text-xxs font-mono font-bold text-text-muted w-5">{number}</span>
    ...
    <div className="space-y-5">
      ...
      <div className="surface p-5 space-y-4">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <h2 className="text-sm font-bold text-text-primary">AI DSA De-coder</h2>
            <p className="text-xxs text-text-muted">Paste any coding problem. AI deconstructs logic, pattern, code, and dry run.</p>
          </div>
        </div>
        ...
        <div className="flex flex-col sm:flex-row gap-2.5">
          <input
            ...
            className="flex-1 surface-inset px-3.5 py-2.5 text-xs text-text-primary placeholder-text-disabled focus:outline-none focus:ring-1 focus:ring-accent rounded-lg"
          />
          <select
            value={language}
            onChange={(e) => setLanguage(e.target.value as ProgrammingLanguage)}
            className="surface-inset px-3 py-2.5 text-xs font-medium text-text-secondary focus:outline-none focus:ring-1 focus:ring-accent rounded-lg"
          >
          ...
          >
            {loading ? <Sparkles className="w-3.5 h-3.5 animate-spin" /> : <Sparkles className="w-3.5 h-3.5" />}
        ...
        <div className="flex flex-wrap items-center gap-1.5">
          ...
          {['Longest Substring Without Repeating Characters', 'Search in Rotated Sorted Array', 'Coin Change', 'Number of Islands'].map((ex) => (
            <button
              ...
              className="px-2 py-1 text-xxs text-text-muted hover:text-accent surface-inset hover:border-accent/20 rounded-md transition-colors"
            >
              {ex}
            </button>
          ))}
      ...
      {loading && (
        <div className="surface p-6 space-y-3">
          <div className="flex items-center gap-3">
            ...
            <span className="text-xs font-medium text-text-secondary">Analyzing problem structure...</span>
          </div>
          <div className="space-y-2">
            {['Parsing constraints', 'Detecting pattern', 'Generating optimal solution', 'Building dry run'].map((step, i) => (
              <div key={i} className="flex items-center gap-2 text-xxs text-text-muted">
                <div className={clsx('w-1.5 h-1.5 rounded-full', i === 0 ? 'bg-accent animate-pulse' : 'bg-bg-elevated')} />
                {step}
              </div>
            ))}
          </div>
      ...
      {result && (
        <div className="space-y-2 animate-fade-in">
          ...
          <Section number="01" title="Problem Restatement" icon={Lightbulb} defaultOpen={true} color="text-accent">
            <p className="text-xs text-text-secondary leading-relaxed">{result.problemRestatement}</p>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mt-3">
              <div className="surface-inset p-3">
                <div className="text-xxs font-semibold text-accent mb-1.5">Inputs</div>
                <ul className="list-disc list-inside text-xs text-text-secondary space-y-0.5">
                  {result.inputs?.map((inp, i) => <li key={i}>{inp}</li>)}
                </ul>
              </div>
              <div className="surface-inset p-3">
                <div className="text-xxs font-semibold text-state-success mb-1.5">Outputs</div>
                <ul className="list-disc list-inside text-xs text-text-secondary space-y-0.5">
          ...
          <Section number="02" title="Constraints & Clues" icon={Target} defaultOpen={true} color="text-accent-amber">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div>
                <div className="text-xxs font-semibold text-accent-amber mb-1.5">Constraints</div>
                <ul className="list-disc list-inside text-xs text-text-secondary space-y-0.5">
                  {result.constraints?.map((c, i) => <li key={i}>{c}</li>)}
                </ul>
              </div>
              <div>
                <div className="text-xxs font-semibold text-accent-violet mb-1.5">Hidden Clues</div>
                <ul className="list-disc list-inside text-xs text-text-secondary space-y-0.5">
          ...
          <Section number="03" title={`Pattern: ${result.pattern}`} icon={Zap} defaultOpen={true} color="text-accent-emerald">
            <p className="text-xs text-text-secondary leading-relaxed">{result.patternWhy}</p>
          </Section>

          {/* 04 Brute Force */}
          <Section number="04" title="Brute Force Approach" icon={AlertTriangle} color="text-accent-rose">
            <div className="flex items-center justify-between mb-2">
              <span className="text-xs text-text-secondary">{result.bruteForce.description}</span>
              <span className="font-mono text-xxs text-accent-rose font-bold">{result.bruteForce.timeComplexity}</span>
            </div>
            <div className="surface-inset p-2.5 text-xxs text-accent-rose">
          ...
          <Section number="05" title="Optimal Approach" icon={CheckCircle2} defaultOpen={true} color="text-state-success">
            <p className="text-xs text-text-secondary leading-relaxed mb-3">{result.optimalApproach.description}</p>
            <div className="surface-inset p-3 font-mono text-xxs text-text-secondary">
              <div className="text-xxs font-semibold text-accent mb-1">Pseudocode</div>
              <pre className="whitespace-pre-wrap">{result.optimalApproach.pseudocode}</pre>
            </div>
            <div className="flex gap-3 mt-2">
              <span className="badge badge-cyan">Time: {result.optimalApproach.timeComplexity}</span>
              <span className="badge badge-emerald">Space: {result.optimalApproach.spaceComplexity}</span>
            </div>
          </Section>
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-left text-xs">
                  <thead className="bg-bg-inset text-text-muted border-b border-border-subtle">
                    <tr>
                      <th className="p-2.5 w-14 font-mono">Step</th>
                      <th className="p-2.5 w-40 font-mono">State</th>
                  ...
                  <tbody className="divide-y divide-border-subtle">
                    {result.dryRun.map((step, idx) => (
                      <tr key={idx} className="hover:bg-white/[0.02]">
                        <td className="p-2.5 font-mono font-bold text-accent">{step.step || idx + 1}</td>
                        <td className="p-2.5 font-mono text-accent-amber">{step.state}</td>
                        <td className="p-2.5 text-text-secondary">{step.explanation}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
          ...
          <Section number="07" title={`Code (${language.toUpperCase()})`} icon={Code2} defaultOpen={true} color="text-accent">
            <div className="flex justify-end mb-2">
              <button onClick={handleCopyCode} className="btn-ghost text-xxs">
                {copied ? <Check className="w-3 h-3 text-state-success" /> : <Copy className="w-3 h-3" />}
                {copied ? 'Copied' : 'Copy'}
              </button>
            </div>
            <pre className="surface-inset p-4 font-mono text-xs text-accent/90 overflow-x-auto leading-relaxed rounded-lg">
              <code>{result.optimalApproach.code}</code>
            </pre>
            {result.optimalApproach.lineByLineExplanation?.length > 0 && (
              <div className="mt-3 space-y-1">
                <div className="text-xxs font-semibold text-text-muted mb-1">Line-by-line</div>
                {result.optimalApproach.lineByLineExplanation.map((line, i) => (
                  <div key={i} className="flex gap-2 text-xxs">
                    <span className="font-mono text-accent shrink-0 w-6 text-right">L{line.line}</span>
