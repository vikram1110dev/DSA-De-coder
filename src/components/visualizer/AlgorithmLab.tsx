    ...
    <div className="space-y-3">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 surface p-3">
        <div className="flex items-center gap-3">
          <h2 className="text-sm font-semibold text-text-primary">{algorithmName}</h2>
          <span className="badge badge-cyan">
            Step {Math.min(currentStepIndex + 1, totalSteps)} / {totalSteps}
          </span>
          {counters && counters.map((c) => (
            <span key={c.label} className="text-xxs font-mono text-text-muted">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[200px_1fr] gap-3">
        ...
        <div className="surface-inset p-6 min-h-[320px] flex flex-col justify-center items-center relative overflow-hidden">
          {/* Subtle glow */}
      ...
      {currentStep && (
        <div className="surface-accent px-4 py-2.5 flex items-start gap-2.5">
          <div className="w-1.5 h-1.5 rounded-full bg-accent mt-1.5 shrink-0 animate-step-pulse" />
